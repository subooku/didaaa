...
static esp_err_t i2s_full_duplex_init(void) {
    // ★ 描述符的划分方式直接决定"供不上 PCM"的风险，别只盯着总容量看：
    //   原来 6 × 240 帧 = 总 180 ms、每个描述符 30 ms。I2S 驱动只有等整个描述符
    //   播完才腾得出空间，于是 i2s_channel_write 被量化成 30 ms 的锯齿：
    //   连着写几块 → 阻塞 30 ms → 再连写几块。阻塞那 30 ms 里存货一路降到 0，
    //   DMA 放空、接上来的波形对不上，就是那一声爆响（实测每 10 秒放空 330 次）。
    //   改成 24 × 64 帧：总容量反而更大（192 ms），每个描述符只有 8 ms，
    //   跟音频任务一块的量对齐 —— 阻塞粒度从 30 ms 降到 8 ms，存货再也降不到 0。
