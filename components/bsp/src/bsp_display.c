...
static esp_err_t backlight_init(void) {
    ...
    s_bl_ready = true;
    // 回读实际频率：LEDC_AUTO_CLK 选到 RC_FAST（约 20 MHz）时 10 bit 最高只有
    // 19.5 kHz，配不上 25 kHz 会静默退到别的时钟，啸叫就又回来了。
    ESP_LOGI(TAG, "背光 LEDC 就绪 gpio=%d  %u Hz", BSP_LCD_BL,
             (unsigned)ledc_get_freq(BSP_BL_LEDC_MODE, BSP_BL_LEDC_TIMER));
    return ESP_OK;
}
